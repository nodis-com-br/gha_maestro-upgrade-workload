...
git clone https://${GITHUB_GLOBAL_TOKEN}@github.com/${GITHUB_REPOSITORY_OWNER}/${MAESTRO_REPOSITORY}.git
cd ${MAESTRO_REPOSITORY}

RESOURCE_FILE=`find . -name ${NODIS_PROJECT_NAME}.yaml`
NEW_VALUES=`printf '{"%s":{"image": {"tag": "%s"}}}' ${NODIS_DEPLOY_ENV} ${NODIS_PROJECT_VERSION}`
